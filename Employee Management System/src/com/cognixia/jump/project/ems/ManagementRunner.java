...
public class ManagementRunner extends Departments implements Initial, Filtered{
	...
	public static void operation(Hashtable<String,ArrayList<Employee>> hashtable, int operation) {
		...
		case 1:
			...
			break;
		case 2://add employee
			String[] promptFor = {"first name", "last name", "salary", "employee id",
					"position", "email"};
			String userInput = "";
			int i = 0;
			Employee newEmployeeObj = new Employee();
			
			for (String attribute : promptFor) {
				System.out.println("Please enter a " + attribute);
				userInput = input.nextLine();
//				System.out.println(i);
				
				newEmployeeObj.updateEmployee(i, userInput);
				i++;	
			}
			
			String fileName = departmentDir.get(departmentKey) + ".txt";
			try(FileWriter fw = new FileWriter(fileName, true);
				    BufferedWriter bw = new BufferedWriter(fw);
				    PrintWriter out = new PrintWriter(bw))
				{
				    out.println(newEmployeeObj.toString());
				} catch (IOException e) {
